...
define([],
    function () {
        ...
        function _updateView(game) {
            _updateTeam(game['myTeam']);
            _updateTeam(game['theirTeam']);
        }
        ...
        function _createGame(game) {
            let el = _createTeam(game['myTeam']) + _createTeam(game['theirTeam']);

            $(".game-details").append(el);
        }

        function _createTeam(team) {
            // Start team element
            let el =
                `<div id="team-${team['color']}" class="team row justify-content-center mx-auto team-${team['color']}">`
            for (let participant of team['participants']) {
                el += _createParticipant(participant, team['color']);
            }
            // end team element
            el += `</div>`
            return el;
        ...
        // TODO: make exception for aphelios
        function _createAbilitiesEl(participant, teamColor) {
            let el =
