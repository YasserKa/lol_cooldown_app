define([],
    function () {
        ...
        function _updateView(game) {
            if (game.hasOwnProperty('blueTeam')) {
                _updateTeam(game['blueTeam'], 'blue');
            }
            if (game.hasOwnProperty('redTeam')) {
        ...
        function _updateTeam(team, color) {
            for (let participant of team) {
                // Update champion
                $(`table[partic-id="${participant['name']}"] .champ-icon`)
                    .attr('src', participant['champion']['icon'])
                    .attr('alt', participant['champion']['name']);

                if ($(`table[partic-id="${participant['name']}"] .cooldowns-abilities`).length > 0) {
                    $(`table[partic-id="${participant['name']}"] .cooldowns-abilities`).remove();
                }

                // Champ Abilities
                if (participant['champion']['abilities'].length !== 0 &&
                    $(`table[partic-id="${participant['name']}"] .cooldowns-abilities`).length === 0) {
                    $(`table[partic-id="${participant['name']}"]`).append(_createAbilities(participant, color));
                }

                // Update spells
                $(`table[partic-id="${participant['name']}"] div.spell-1 img`)
                    .attr('src', participant['spells'][0]['image'])
                    .attr('alt', participant['spells'][0]['name']);

                $(`table[partic-id="${participant['name']}"] div.spell-1 p`)
                    .attr('spell-name', participant['spells'][0]['name'])
                $(`table[partic-id="${participant['name']}"] div.spell-1 p span`)
                    .text(participant['spells'][0]['cooldown']);

                $(`table[partic-id="${participant['name']}"] div.spell-2 img`)
                    .attr('src', participant['spells'][1]['image'])
                    .attr('alt', participant['spells'][1]['name']);

                $(`table[partic-id="${participant['name']}"] div.spell-2 p`)
                    .attr('spell-name', participant['spells'][1]['name'])
                $(`table[partic-id="${participant['name']}"] div.spell-2 p span`)
        ...
        function _createParticipant(participant, teamColor) {
            let el =
                `<table class="champ" partic-id="${participant['name']}">
            <tbody><tr>
